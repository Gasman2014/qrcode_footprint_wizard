...
class QRCodeWizard(FootprintWizardBase.FootprintWizard):
    ...
    def GenerateParameterList(self):
        ...
        self.AddParam("Barcode", "Contents", self.uString, 'Sample')
        ...
        self.AddParam("Caption", "Enabled", self.uBool, False)
    ...
    def _drawPixel(self, xposition, yposition):
        ...
        pad.SetPadName("1")
        self.module.Add(pad)
        if (self.UseSilkS and self.onBack):
            polygon=self.drawSquareArea(pcbnew.B_SilkS, self.X, xposition, yposition)
    ...
    def BuildThisFootprint(self):
            ...
            for line in arrayToDraw:
                ...
                for pixel in line:
                    ...
                    if self.negative != pixel: # Xor...
                        self._drawPixel(self.qr.modules.__len__() - xposition, yposition)
                    xposition += self.X
                yposition += self.X
        else:
            for line in arrayToDraw:
        ...
        if self.onBack:
            ...
            self.module.Value().SetLayer(pcbnew.B_SilkS)
        else:
            self.module.Value().SetPosition(pcbnew.wxPoint(0, - textPosition))
            self.module.Reference().SetPosition(pcbnew.wxPoint(0, textPosition))
